...
public class Tela5 extends AppCompatActivity {
    ...
    public void resposta1() {
        ...
        if (selectedId == -1) {
            Toast.makeText(Tela5.this, "Por favor, selecione uma resposta!", Toast.LENGTH_SHORT).show();
            return;
        }

        RadioButton selectedRadioButton = findViewById(selectedId);
        String respostaEscolhida = selectedRadioButton.getText().toString();
        String respostaCorreta = "Obras";

        int acertos = getIntent().getIntExtra("acertos", 0);

        if (respostaEscolhida.equals(respostaCorreta)) {
            Toast.makeText(Tela5.this, "Resposta correta!", Toast.LENGTH_SHORT).show();
            acertos++; // Correção aqui
        } else {
            Toast.makeText(Tela5.this, "Resposta errada!", Toast.LENGTH_SHORT).show();
        }
        abrirTela2(acertos);
    }
