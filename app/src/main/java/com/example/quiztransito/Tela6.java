...
public class Tela6 extends AppCompatActivity {
    ...
    public void resposta1() {
        ...
        if (selectedId == -1) {
            Toast.makeText(Tela6.this, "Por favor, selecione uma resposta!", Toast.LENGTH_SHORT).show();
            return;
        }

        RadioButton selectedRadioButton = findViewById(selectedId);
        String respostaEscolhida = selectedRadioButton.getText().toString();
        String respostaCorreta = "Proibido ultrapassar";

        int acertos = getIntent().getIntExtra("acertos", 0);

        if (respostaEscolhida.equals(respostaCorreta)) {
            Toast.makeText(Tela6.this, "Resposta correta!", Toast.LENGTH_SHORT).show();
            acertos++; // Correção aqui
        } else {
            Toast.makeText(Tela6.this, "Resposta errada!", Toast.LENGTH_SHORT).show();
        }
        abrirTela2(acertos);
    }
    ...
    private void abrirTela2(int acertos) {
        Intent intent = new Intent(Tela6.this, TelaResultado.class);
